...
sub matrix($$$$$$$$$) {new($_[0],$_[1],$_[2],$_[3],$_[4],$_[5],$_[6],$_[7],$_[8])};
...
sub matrixVectorMultiply($$)
 {my ($a) =         check(@_[0..0]); # Matrix
  my ($b) = vector::check(@_[1..1]); # Vector 
  vector
...
sub multiply3
 {my ($a, $b) = @_;
  ...
  return $a->matrixVectorMultiply($b) if vector::is($b);
...
sub divide3
 {my ($a, $b, $c) = @_;
  if (!ref($b))
   ...
   {return $a->inverse->matrixVectorMultiply($b) if vector::is($b);
