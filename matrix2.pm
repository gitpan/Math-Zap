...
sub matrix2($$$$) {new($_[0],$_[1],$_[2],$_[3])};
...
sub matrixVectorMultiply($$)
 {         check(@_[0..0]) if debug; # Matrix
  vector2::check(@_[1..1]) if debug; # Vector 
  my ($a, $b) = @_;
  vector2
   ($a->{11}*$b->{x}+$a->{12}*$b->{y},
    $a->{21}*$b->{x}+$a->{22}*$b->{y},
...
sub multiply3
 {my ($a, $b) = @_;
  ...
  return $a->matrixVectorMultiply($b) if vector2::is($b);
...
sub divide3
 {my ($a, $b, $c) = @_;
  if (!ref($b))
   ...
   {return $a->inverse->matrixVectorMultiply($b) if vector2::is($b);
